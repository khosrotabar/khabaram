...
import styles from "./FavortitesLeftPanel.module.css";

function FavoritesLeftPanel(props) {
  const [userFavorites, setUserFavorites] = useState(
    props.userFavorites ? props.userFavorites : []
  );
  const [showRemoveBtn, setShowRemoveBtn] = useState(false);
  const [showRemoveDialog, setShowRemoveDialog] = useState(false);
  const [removingFavorites, setRemovingFavorites] = useState(false);
  const [removingOneFavorite, setRemovingOneFavorite] = useState(false);
  const [currentIndex, setCurrentIndex] = useState("");
  const [currentIndexByClick, setCurrentIndexByClick] = useState("");
  const editSvg = (
    <svg
      width="23"
      height="6"
  ...
  const trashSvg = (
    ...
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M5.09521 1.15087C5.51196 0.734126 6.07719 0.5 6.66656 0.5H13.3332C13.9226 0.5 14.4878 0.734126 14.9046 1.15087C15.3213 1.56762 15.5554 2.13285 15.5554 2.72222V6.05556C15.5554 6.6692 15.058 7.16667 14.4443 7.16667C13.8307 7.16667 13.3332 6.6692 13.3332 6.05556V2.72222H6.66656L6.66656 6.05556C6.66656 6.6692 6.1691 7.16667 5.55545 7.16667C4.9418 7.16667 4.44434 6.6692 4.44434 6.05556V2.72222C4.44434 2.13285 4.67846 1.56762 5.09521 1.15087Z"
        fill="#FDFDFD"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M0 6.05555C0 5.4419 0.497461 4.94444 1.11111 4.94444H18.8889C19.5025 4.94444 20 5.4419 20 6.05555C20 6.6692 19.5025 7.16667 18.8889 7.16667H17.7778V20.5C17.7778 21.0894 17.5437 21.6546 17.1269 22.0713C16.7102 22.4881 16.1449 22.7222 15.5556 22.7222H4.44444C3.85508 22.7222 3.28985 22.4881 2.8731 22.0713C2.45635 21.6546 2.22222 21.0894 2.22222 20.5V7.16667H1.11111C0.497461 7.16667 0 6.6692 0 6.05555ZM4.44444 7.16667V20.5H15.5556V7.16667H4.44444Z"
        fill="#FDFDFD"
      />
    </svg>
  );

  async function removeAllFavoritesHandler() {
    setRemovingFavorites(true);
    const userMobile = props.userMobile;
    const response = await fetch("/api/removeAllFavorites", {
      method: "POST",
    ...
    setRemovingFavorites(false);
  }

  async function removeFavoritesHandler(code, index) {
    setRemovingOneFavorite(true);
    setCurrentIndexByClick(index);
    const userMobile = props.userMobile;
    const response = await fetch("/api/removeUserFavorite", {
      method: "POST",
    ...
    setRemovingOneFavorite(false);
    setCurrentIndexByClick("");
  }

  return (
    <div className={styles.favorites_left_panel_container}>
      <div
        className={
          showRemoveBtn
            ? `${styles.favorites_main_title} ${styles.show_remove_btn}`
            : styles.favorites_main_title
        }
      >
      ...
      <div className={styles.favorites_grid_container}>
        {userFavorites.map((item, index) => {
          return (
            <div
              key={index}
              className={styles.favorites_container}
              onMouseOver={() => setCurrentIndex(index)}
              onMouseOut={() => setCurrentIndex("")}
            >
              <div className={styles.favorites_right}>
                <div className={styles.favorites_image}>
                  <Image
                    alt=""
                    layout="responsive"
                    width="1"
              ...
              <div className={styles.favortites_left}>
                <p
                  className={styles.favortites_sub_title}
                  style={
                    currentIndex === index || currentIndexByClick === index
                      ? { color: "#585858" }
                      : {}
                  }
                >
                ...
                <div className={styles.favortites_price}>
                  <p
                    style={
                      currentIndex === index || currentIndexByClick === index
                        ? { color: "#585858" }
                ...
                <div
                  className={
                    currentIndex === index || currentIndexByClick === index
                      ? `${styles.favortites_similar_products_link} ${styles.show_hidden_element}`
                      : styles.favortites_similar_products_link
                  }
                >
                  <p>کالاهای مشابه</p>
                  {arrowSvg}
                ...
                <button
                  className={
                    currentIndex === index || currentIndexByClick === index
                      ? `${styles.favortites_remove_btn} ${styles.show_hidden_element}`
                      : styles.favortites_remove_btn
                  }
                  onClick={() => removeFavoritesHandler(item.code, index)}
                >
                  {removingOneFavorite && currentIndexByClick === index
                    ? undefined
                    : trashSvg}
                  {removingOneFavorite &&
                  currentIndexByClick === index ? undefined : (
                    <p>حذف کالا</p>
                  )}
                  <BeatLoader
                    color="#fff"
      ...
      >
        ...
        >
          ...
          <div className={styles.delete_dialog_form_up_side}>
            ...
            <p>حذف علاقه مندی ها</p>
          </div>
          <div className={styles.delete_dialog_form_down_side}>
            <p>آیا از حذف تمام علاقه مندی ها اطمینان دارید؟</p>
            <div>
              <button
                onClick={
                  removingFavorites ? undefined : removeAllFavoritesHandler
                }
              >
                {removingFavorites ? (
                  <div className={styles.favorites_loading_2}>
                    <Image
                      alt=""
                      layout="responsive"
                      width="1"
...
export default FavoritesLeftPanel;

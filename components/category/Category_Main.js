...
import styles from "./Category_Main.module.css";

function CategoryMain(props) {
  const [showLoadingSkeleton, setShowLoadingSkeleton] = useState(true);
  const [allProducts, setAllProducts] = useState([]);
  const [hasMore, sethasMore] = useState(true);
  const brandFilter = useSelector((state) => state.categoryFilter[0]);
  const priceFilterObject = useSelector((state) => state.categoryFilter[1]);
  const priceFilterArray = [
    priceFilterObject.priceFrom,
    priceFilterObject.priceTo,
  ];
  const stockCondition = useSelector((state) => state.categoryFilter[2]);
  const upSideFilter = useSelector((state) => state.categoryFilter[3]);
  ...
  const filteredProducts =
    upSideFilter.upSideFilter === "newer"
      ? allProducts.sort(
          (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
        )
      : upSideFilter.upSideFilter === "highPrice"
      ? allProducts.sort((a, b) => b.final_price - a.final_price)
      : upSideFilter.upSideFilter === "lowPrice"
      ? allProducts.sort((a, b) => a.final_price - b.final_price)
      : allProducts;
  ...
  async function getMorePost() {
    const response = await fetch("/api/category", {
      ...
      body: JSON.stringify({
        slug: props.slug,
        greaterThan: allProducts.length,
      }),
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const otherProducts = await response.json();
    setAllProducts((products) => [...products, ...otherProducts]);
  ...
  async function favoritesHandler(title, price, final_price, src, code) {
    if (userMobileNumber) {
      const response = await fetch("/api/storeInFavorites", {
        method: "POST",
        body: JSON.stringify({
          userMobileNumber,
          title,
      ...
      // for now. later must be complete
      console.log(results);
    } else {
  ...
  return (
    <>
      ...
      >
        <div className={styles.card_loading} style={{ marginBottom: "20px" }}>
      ...
      <InfiniteScroll
        ...
        hasMore={hasMore}
        loader={
          <div className={styles.loadingio_spinner_pulse_t8u72195txo}>
            <div className={styles.ldio_4b0w5ifgarp}>
              <div></div>
              <div></div>
              <div></div>
            </div>
        ...
        style={showLoadingSkeleton ? { display: "none" } : { display: "block" }}
      >
        <div className={styles.category_main}>
          {filteredProducts.map((product) => {
            const scoreArray = product.score;
            var j = 0;
            for (var i = 0; i < scoreArray.length; i++) {
              j += scoreArray[i].score;
            }

            const scoreMean = j / scoreArray.length;

            return brandFilter.brand === product.brand &&
              priceFilterArray[0] <= product.price &&
              product.price <= priceFilterArray[1] &&
              (stockCondition.stock == 1 ? product.stock > 0 : true) ? (
              <div
                key={product.id}
                className={styles.category_product_container}
              >
                  ...
                  <div className={styles.product_discount}>
                    ...
                    >
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M15.4652 15.4652C14.8412 16.0892 13.8296 16.0892 13.2057 15.4652L12.0759 14.3355C11.452 13.7115 11.452 12.6999 12.0759 12.076C12.6999 11.452 13.7115 11.452 14.3354 12.076L15.4652 13.2057C16.0891 13.8297 16.0891 14.8413 15.4652 15.4652ZM16.5949 0.778488C17.2189 1.40243 17.2189 2.41404 16.5949 3.03798L3.03795 16.595C2.41401 17.2189 1.40239 17.2189 0.778452 16.595C0.15451 15.971 0.15451 14.9594 0.778453 14.3355L14.3354 0.778488C14.9594 0.154545 15.971 0.154545 16.5949 0.778488ZM5.29744 5.29748C4.6735 5.92142 3.66189 5.92142 3.03795 5.29748L1.9082 4.16773C1.28426 3.54379 1.28426 2.53218 1.9082 1.90824C2.53214 1.28429 3.54375 1.28429 4.1677 1.90824L5.29744 3.03798C5.92139 3.66193 5.92139 4.67354 5.29744 5.29748Z"
                        fill="#F45858"
                      />
                    </svg>
                  </div>
                ...
                <Link href={`/product/${product.url}`} target="_blank">
                  <div className={styles.category_product_image_container}>
                    {product.stock == 0 ? (
                      <div className={styles.stock_free_icon}>ناموجود</div>
                    ) : undefined}
                    <div className={styles.category_product_image}>
                      <Image
                ...
                <div
                  className={`${styles.category_product_title_score} ${
                    product.like == 1 ? styles.active_like : undefined
                  }`}
                >
                  <Link href={`/product/${product.url}`} target="_blank">
                    <p className={styles.category_product_title}>
                      {product.title}
                    </p>
                  </Link>

                  <div
                    className={styles.category_product_score}
                    onClick={() =>
                      favoritesHandler(
                        product.title,
                        product.price,
                  ...
                  >
                    <p style={{ color: "#b5b5b5" }}>
                      {scoreMean ? scoreMean : "5"}
                    </p>
                    {ScoreSvg}
                  </div>
                </div>
                <div className={styles.category_product_price}>
                  <div>
                    <p>
                      {product.discount !== ""
                        ? numberWithCommas(product.final_price)
